...
| `{{PREAMBLE}}` | `gen-skill-docs.ts` | Startup block: session tracking, contributor mode, AskUserQuestion format |
...
2. **Contributor mode** — reads `gstack_contributor` from config. When true, the agent files casual field reports to `~/.gstack/contributor-logs/` when gstack itself misbehaves.
...
4. **Search Before Building** — before building infrastructure or unfamiliar patterns, search first. Three layers of knowledge: tried-and-true (Layer 1), new-and-popular (Layer 2), first-principles (Layer 3). See `ETHOS.md` for the full builder philosophy.
